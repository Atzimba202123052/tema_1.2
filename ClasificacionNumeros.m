% De secuencia de comando de octave
% Titulo: Clasificacion de los numeros
% Descripcion: Script para recordar conceptos de numeros  
% Autor: Atzimba Joice Martinez Huitron 
% Fecha: 30092021
% Vercion: 1 
% Uso: Octave> / ruta / ClasificacionNumeros 
% Notas: Requiere aplicacion octave, usar la linea de comandos
%                  :  https://octaveintro.readthedocs.io/en/latest/index.html
 

% Clear 
c_numeros_Naturales = ' N = {1, 2, 3, ...n} si n> 0 ' ; 
c_numeros_Enteros = ' Z = { -n ..., -2, -1, 0, 1, 2, ...n} ' ;
c_numeros_Racionales = ' Q = { m / n donde m, n, ? R n? 0 } ' ;
c_numeros_Irracionales = ' I = { vn  que no puede ser expresada como Q todas las raices qwue no son exactas} ' ; 
c_numeros_Reales = 'R = { I, Q, Z, N,} ' ;
 
% Propiedades de los numeros, sean a, b, c, d, e? R 

% Propiedades de? (Cerradura) 
p_cerradura = ' a - b? R ';
p_cerradura = ' ab? R ' ; 
p_cerradura3 = ' 7 + 9? N '; 
p_cerradura = ' ? = permanencia ' ;
a = 3 ;
b = 5 ;
a + b 
a + b 


% Propiedad asociativa 
p_asociativa = ' a + (b + c) ' ; 
p_asociativa2 = ' a (bc) =(ab) c ' ; 
p_asociativa3 = ' 3 + (8 - 10) = (8 + 3) -10 ' ;
A = 5 ;
B = 6 ; 
C = 3 ;
A +(B + C)  
(A - B) + C 

% Propiedad conmitativa 
p_conmitativa = ' a + b = b + a ' ;
p_conmitativa2 = ' ab = ba ' ;
a = 8 ;
b = 3 ; 
a + b 
b + a 

% Propiedad distributiva 
p_distributiva = ' a ( b + c) = ab + ac ' ;
a = 4 ;
b = 8 ; 
c = 6 ; 
a = (b + c)  
(a - b) + (a - c) 

% Neutro auditivo 
p_neutroA = ' a + 0 = a ' ; 
p_neutroA2 = ' Ojo: a  + 0 = 0 + a ---> es conmutativa ' ; 
a = 4 ;
a + 0 
% Neutro multiplicativo 
p_neutroM = ' a (1) = a ' ;
a = 7 ;
a = 1 
 
% Inverso aditivo 
p_inversoA = ' a + -a = 0 ' ; 
a = 2 ; 
a + (- a) 

% Inverso multiplicativo o reciproco 
p_InversoM = ' a (1 / a) = 1 ' ;
a = 5 ; 
a = 1 / a 

% Propiedad Trancitiva (| entones) 
p_trancitiva = ' si a> byb> c | a> c ' ;
p_trancitiva2 = ' si a = bye = c | a = c ' ; 
a = 5 ; 
b = 3 ;
c = 1 ; 
a> b> c | a> c 


% Tricotomia (rais del algebra) siempre se pueden comparar 
p_tricotomia = ' a> b' ; 
p_tricotomia2 = 'a = b ' ; 
p_tricotomia3 = ' a <b ' ; 
a = 3 ; 
b = 2 ; 
a> b 

% Signos de agrupacion 
s_agrupacion = ' {{( )}} ' ; 